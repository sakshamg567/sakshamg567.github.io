...
const App = () => {
  ...
  const skillBadge = (items) => {
    ...
    return (
      items.map((item, i) => (
        <Badge key={i} variant={"outline"} className="border border-zinc-700 text-gray-400">{item}</Badge>
      ))
    )
  }
  return (
    <div className='bg-black'>
      <main className={`flex min-h-screen flex-col text-white p-8 md:p-16 lg:p-24 max-w-3xl mx-auto`}>
        <div className='mt-10 md:mt-0 mb-10 flex flex-row items-center gap-5 rounded-md place-content-between'>
          <div className='flex flex-row gap-5 items-center'>
            <div className='overflow-hidden flex items-center justify-center w-20 rounded-full'>
              <img src="/pfp_x_ghibli.jpg" alt="" className='object-cover w-full h-full' />
            </div>
            <div className='scale-95 flex flex-col'>
              <span className='font-sans font-semibold text-4xl'>Saksham</span>
              <span className='font-mono text-zinc-500 leading-relaxed tracking-normal'>@skshmgpt</span>
            </div>
          </div>
          <div className='flex flex-row items-center gap-4 scale-90'>
        ...
        <section>
          <div className="text-gray-400 leading-relaxed tracking-wide font-mono text-sm">
            <h1 className="font-bold text-2xl mb-4 text-white">Skills</h1>
            {Object.entries(skills).map(([category, items]) => (
              <div key={category} className="mb-4 text-white">
                <span className="text-white mb-2 mr-3">{category}</span>
        ...
        <section id="Projects">
          <div className="text-gray-400 leading-relaxed tracking-wide font-mono text-sm">
            <h1 className="font-bold text-2xl text-white mb-4">Projects</h1>
        ...
        <hr className="my-5 border-zinc-800" />
        <section id='contact'>
          <div className='flex flex-row justify-center gap-6 text-gray-400 leading-relaxed tracking-wide font-mono text-sm'>
            <Social
              Icon={GitHub}
              Text={"Github"}
